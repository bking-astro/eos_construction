...
def sample_ns(num_points, n_start, max_n):
    ...
    #add large point for interpolation
    ns = np.append(ns, 40*n0)
...
def sample_cs(num_points, cs_start):
    ...
    # add standard cs for interpolation high end
    cs = np.append(cs, 0.333333)
...
def extend(small_EOS, nsamp, size=None, rho_step=1e-3, rho_max=10, ext_type=None, max_gamma=3):
    ...
    if ext_type == 'cs' or ext_type == 1:
        ...
        causality = False # set false so loop will run
        while causality == False:
            # extend EOS and will loop untill we get EOS_ex that obeys causality
